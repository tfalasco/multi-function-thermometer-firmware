...
#define HIGH_POWER_MODE
#define RITS_DELTA				(0.3)

#define DEFAULT_AUTO_SLEEP		20
#define DEFAULT_SLEEP_TIME		60

#define FIRMWARE_VER			"2.12"	// Must be 4 characters
...
#else
#define HARDWARE_VER			"008"	// Must be 3 characters
#define DEVICE_NAME				"Blue2-MFT"
#define DEVICE_NAME_LEN			9
#endif //INCLUDE_IR
...
void LoadBigDisplayBuffer(uint8 buffer[], char message[14]);
void LoadMacDisplayBuffer(uint8 *buffer, char message[14]);
//void LoadBottomDisplayBuffer(uint8 *buffer, char leftMessage[6], char rightMessage[6]);
void LoadStatusIconBuffer(uint8 buffer[], char message[14]);
...
//	char modeMessage[14] = {0x00};
	char statusIconMessage[14] = {0x00};
//	char leftMessage[6] = {0x00};
//	char rightMessage[6] = {0x00};
...
//	uint16	stabilizationTimer;
//	ItsValue = 1;	// MicroNeedle
//	ItsValue = 2;	// DuraNeedle
//	bool changeScale = false;

//	uint8 numRecords = 0;
//	float records[4];
//	uint8 displayRecord = 0;
//	bool isStable = false;
//	bool ritsFlash = true;
//	bool ritsLockout = false;
//	uint8 RitsModeBuffer[1];
	uint8 NotOkayToRecordBuffer[1] = {0};
//	bool showMemClear = false;

	float antepenultimateTemp[1] = {25.0f};	// In C
	float previousTemp[1] = {25.0f};	// In C
//	float lastStableTemp[1] = {25.0f};	// In C, used for connected RITS mode
	bool okayToRecord = false;
	bool LockedOn;
	bool HasMoved;
	float LockedTemp;
	bool FlashOffLockedTemp;

	bool updateBlankLines;
	bool updateMACLine;
	bool updateStatusLine;
	bool updateTempLine;
	bool updateBatteryLine;
...
//	uint8 dataToWrite[1];
	...
	AutoSleepInterval = DEFAULT_AUTO_SLEEP;
	...
	while (1)
	{
		...
		switch (BGLIB_MSG_ID(evt->header))
		{
			case gecko_evt_system_boot_id:
				...
				updateBlankLines = true;
				updateStatusLine = true;
				updateMACLine = true;
				updateTempLine = true;
				updateBatteryLine = true;
				...
				{
					AutoSleepInterval = DEFAULT_AUTO_SLEEP; // Default Value
					uint8 data[1];
				...
				if (loadRsp->result == ps_key_not_found)
				{
					DisplayScale = 0; // Default Value (C)
					uint8 data[1];
					data[0] = DisplayScale;
				...
				// RITS Characteristics
				NotOkayToRecordBuffer[0] = 0;
				gecko_cmd_gatt_server_write_attribute_value(gattdb_rits_not_okay_to_record_characteristic, 0, 1, NotOkayToRecordBuffer);				// Update the GATT database
				...
				else
				{
					...
					gecko_cmd_le_gap_set_adv_data(1, 2 + DEVICE_NAME_LEN, ScanResponseData);
				}
...
//				StateMachine = smOFF;
			...
			case gecko_evt_gatt_server_attribute_value_id:
				...
				if (evt->data.evt_gatt_server_user_write_request.characteristic == gattdb_rits_unlock_characteristic)
				{
					if (okayToRecord)
					{
						WorkingClean = false;
						ResetAutoSleep(&autoSleepTimer, &sleepingTimer, measurementInterval);
						LockedOn = false;
#ifdef HIGH_POWER_MODE
						gecko_cmd_hardware_set_soft_timer(tmTENMILLISEC, thMEASUREMENT, 1);
#endif  // HIGH_POWER_MODE

					}
					else
					{
						BuzzerProfile = bpDOUBLE_BEEP;
						buzzerStep1 = 1;
					}
				}
				...
				if (evt->data.evt_gatt_server_user_write_request.characteristic == gattdb_legacy_auto_off_interval)
				{
					// For legacy Blue2 support
					// Auto-off Interval
					if (evt->data.evt_gatt_server_user_write_request.value.len == 1)
				...
				else if (evt->data.evt_gatt_server_user_write_request.characteristic == gattdb_display_scale_characteristic)
				{
					if (evt->data.evt_gatt_server_user_write_request.value.len == 1)
					{
							...
							uint8 data[1] = { 0 };
							gecko_cmd_flash_ps_save(psDISPLAY_SCALE, 1, data);
							DisplayScale = 0;gecko_cmd_gatt_server_write_attribute_value(gattdb_display_scale_characteristic, 0, 1, data);				// Update the GATT database
				...
				else if (evt->data.evt_gatt_server_user_write_request.characteristic == gattdb_measurement_interval_characteristic)
				{
					if (evt->data.evt_gatt_server_user_write_request.value.len == 1)
					{
						...
						/* We are measuring at 1/2 second intervals for RITS
...
//					}
				...
				if (evt->data.evt_hardware_soft_timer.handle == thAUTO_SLEEP)
				{
					...
					if (autoSleepTimer >= AutoSleepInterval)
					{
						...
						else if ((StateMachine  == smSTANDALONE) || (StateMachine  == smADVERTISING))
						{
							ShutDownTasks(&BuzzerProfile, &ConnectionHandle, &StateMachine, &advertisementTimer, &autoSleepTimer);
						}
					}
...
//						if ((StateMachine != smOFF) && (WorkingClean == true))
						if (StateMachine != smOFF)
						{
#ifndef HIGH_POWER_MODE
							gecko_cmd_hardware_set_soft_timer(tmQUARTERSECOND, thWAIT_FOR_VMEAS_TC, 1); 	// Wait  for VMeasTC2.7
#else
							gecko_cmd_hardware_set_soft_timer(tmTENMILLISEC, thWAIT_FOR_VMEAS_TC, 1); 	// Wait  for VMeasTC2.7
#endif  // HIGH_POWER_MODE
						}
						...
						if (resettingMeasTimer)
						{
							...
							{
#ifndef HIGH_POWER_MODE
								gecko_cmd_hardware_set_soft_timer(tmSECOND / 2, thMEASUREMENT, 0);
#endif  // HIGH_POWER_MODE
							}
						}
					}
...
//				if (evt->data.evt_hardware_soft_timer.handle == thRITS_FLASH)
//				{
//						ritsFlash = !ritsFlash;
...
//				}
				if (evt->data.evt_hardware_soft_timer.handle == thCALC_TEMP)
				{
					...
					if ((StatusRegister[0] & 0x08) == 0x00)									// Not in calibration mode
					{
						if (WorkingClean == true)
						{
							...
							if (LockedOn == false)	// Not Locked On
							{
								...
								{
									if ((data.val < LockedTemp - 1.7) && (WorkingClean)) // Has temp dropped 3 degs F since locking on?
									{
										HasMoved = true;
									}

									if ((data.val >= 60) && (data.val <= 93.3))	// In RITS Range
									{
										if (HasMoved == true) // Temp has dropped 3 degs F
										{
											// Check if stable
											float delta1[1] = {previousTemp[0] - antepenultimateTemp[0]};
											if (delta1[0] < 0)
											{
												delta1[0] *= -1;
											}
											float delta2[1] = {data.val - previousTemp[0]};
											if (delta2[0] < 0)
											{
												delta2[0] *= -1;
											}
											...
											if ((delta1[0] <= RITS_DELTA) && (delta1[0] >= -RITS_DELTA))	// Stable
											{
												okayToRecord = true;
												StatusRegister[1] &= 0xBF;			// Clear Not okay to record flag

												LockedOn = true;
												HasMoved = false;
												LockedTemp = data.val;
												FlashOffLockedTemp = true;
#ifdef HIGH_POWER_MODE
												GPIO_PortOutSetVal(gpioPortC, tcOFF, tcMASK);							// Turn off VMeasTC2.7
#endif  // HIGH_POWER_MODE
												gecko_cmd_hardware_set_soft_timer(tmTENTHSECOND, thRITS_FLASH_ON, 1);
											}
											else // Not stable
											{
												okayToRecord = false;
												StatusRegister[1] |= 0x40;			// Set Not okay to record flag
											}
										}
										else // Has not dropped 3 degs F yet
										{
											okayToRecord = false;
											StatusRegister[1] |= 0x40;			// Set Not okay to record flag
										}
									}
									else // Not in RITS range
									{
										okayToRecord = true;
										StatusRegister[1] &= 0xBF;			// Clear Not okay to record flag
									}
									...
									antepenultimateTemp[0] = previousTemp[0];
									previousTemp[0] = data.val;
								...
								// Set or clear the Okay to Record flag in GATT
								NotOkayToRecordBuffer[0] = !okayToRecord;
								gecko_cmd_gatt_server_write_attribute_value(gattdb_rits_not_okay_to_record_characteristic, 0, 1, NotOkayToRecordBuffer);				// Update the GATT database
								if (StateMachine == smCONNECTED)
								{
									gecko_cmd_gatt_server_send_characteristic_notification(ConnectionHandle, gattdb_rits_not_okay_to_record_characteristic, 1, NotOkayToRecordBuffer);	// Send notification
								}
							}
						}
#ifdef HIGH_POWER_MODE
				...
				if (evt->data.evt_hardware_soft_timer.handle == thRITS_FLASH_ON)
				{
					FlashOffLockedTemp = false;
					...
					if (StateMachine != smOFF)
					{
						if (LockedOn)
						{
							gecko_cmd_hardware_set_soft_timer(tmQUARTERSECOND * 2, thRITS_FLASH_OFF, 1);
							updateTempLine = true;
							gecko_cmd_hardware_set_soft_timer(tmTENMILLISEC, thDISPLAY, 1);
						}
					}
				...
				if (evt->data.evt_hardware_soft_timer.handle == thRITS_FLASH_OFF)
				{
					FlashOffLockedTemp = true;
...
//						filledRow[i] = 0x00;
					...
					else if ((StateMachine == smCONNECTED) || (StateMachine == smSLEEPING))
					{
						// Blank Lines
						if (updateBlankLines)
						{
...
//					}
					...
					if (updateStatusLine)
					{
						...
						if (okayToRecord)
						{
							statusIconMessage[5] = symOKAY_LEFT;
							statusIconMessage[6] = symOKAY_RIGHT;
						}
						else
						{
							statusIconMessage[5] = symNOT_OKAY_LEFT;
							statusIconMessage[6] = symNOT_OKAY_RIGHT;
						}

						statusIconMessage[7] = 0x00;

	//					sprintf(statusIconMessage, "%c%c%c%c%c%c%c%c%c", symNO_CAL1, symBLANK5, symBATT_ERR, symBLANK5, symBLANK, symBLANK5, symNO_PROBE, symBLANK5, symNO_PROBE1);

						// Display the bluetooth string
						LoadStatusIconBuffer(tempLine, statusIconMessage);
						rowPointerPlaceholder = rowPointer;
						for (; rowPointer < 54; rowPointer++)
						{
							tempLine[((rowPointer - rowPointerPlaceholder) * 18) + 0] = ReverseBits(rowPointer + 1);
							tempLine[((rowPointer - rowPointerPlaceholder) * 18) + 17] = 0x00;
							for (uint8 byte = 0; byte < 18; byte++)
					...
					if (updateTempLine)
					{
						...
						if (LockedOn && FlashOffLockedTemp)
						{
							sprintf(tempDisplayMessage, " ");
							useBigTemp = true;
						}
						else if (StateMachine == smSLEEPING)
						{
							sprintf(tempDisplayMessage, "!");
							useBigTemp = true;
						}
	...
	//					{
							// Temp Line
							LoadBigDisplayBuffer(tempLine, tempDisplayMessage);
							rowPointerPlaceholder = rowPointer;
							for (; rowPointer < 107; rowPointer++)
							{
								tempLine[((rowPointer - rowPointerPlaceholder) * 18) + 0] = ReverseBits(rowPointer + 1);
								tempLine[((rowPointer - rowPointerPlaceholder) * 18) + 17] = 0x00;
								for (uint8 byte = 0; byte < 18; byte++)
	...
	//
	//						// Temp Line
	//						LoadDisplayBuffer(tempLine, tempDisplayMessage);
	//						rowPointerPlaceholder = rowPointer;
	//						for (; rowPointer < 95; rowPointer++)
	//						{
	//							tempLine[((rowPointer - rowPointerPlaceholder) * 18) + 0] = ReverseBits(rowPointer + 1);
	//							tempLine[((rowPointer - rowPointerPlaceholder) * 18) + 17] = 0x00;
	//							for (uint8 byte = 0; byte < 18; byte++)
	...
	//					}
					...
					// Battery Icon Line
					if (updateBatteryLine)
					{
						updateBatteryLine = false;
						AddBatteryIcon(tempLine, battPercentage);
						rowPointerPlaceholder = rowPointer;
						for (; rowPointer < 115; rowPointer++)
						{
							tempLine[((rowPointer - rowPointerPlaceholder) * 18) + 0] = ReverseBits(rowPointer + 1);
							tempLine[((rowPointer - rowPointerPlaceholder) * 18) + 17] = 0x00;
							for (uint8 byte = 0; byte < 18; byte++)
					...
					case splSHORT:
						...
						case ssSW1_PRESSED:
							...
							case smOFF:
								...
								// Clear Not okay to record flag in GATT
								NotOkayToRecordBuffer[0] = !okayToRecord;
								gecko_cmd_gatt_server_write_attribute_value(gattdb_rits_not_okay_to_record_characteristic, 0, 1, NotOkayToRecordBuffer);				// Update the GATT database
...
//							case smOFF:
//							{
//								// Change scale and then start in RITS mode if no records, or SCROLL mode if there are records.
//								uint8 dataToWrite[1];
//
//								// Change scale
...
//	double calculatedTemperature, adcCountscalc;
	uint8 tcIndex;
...
/*
	// US Sensor
	// The formula for converting from TM counts to *C was
	// calculated in Excel using a 5th order polynomial fit.
	// The r^2 value is 0.99998941985358
	// y = -0.000000000000000001298193x5
	// + 0.000000000000050475629219x4
	// - 0.000000000789161556387307x3
	// + 0.000006383185408318580000x2
	// - 0.031840817951645600000000x
	// + 95.047222630045200000000000
	adcCountscalc = adcCounts;
	calculatedTemperature = (-0.000000000000000001298193 * adcCountscalc * adcCountscalc * adcCountscalc * adcCountscalc * adcCountscalc) +
			(0.000000000000050475629219 * adcCountscalc * adcCountscalc * adcCountscalc * adcCountscalc) -
			(0.000000000789161556387307 * adcCountscalc * adcCountscalc * adcCountscalc) +
			(0.000006383185408318580000 * adcCountscalc * adcCountscalc) -
			(0.0318408179516456 * adcCountscalc) +
			+ 95.0472226300452;
	*Temperature = (int32)calculatedTemperature * 1000; 					// Temperature is now the thermistor temp in Celsius x 1000
*/
...
/*
	...
	// The formula for converting from TM counts to *C was
	// calculated in Excel using a 5th order polynomial fit.
	// The r^2 value between -30 and 60 C is 0.999933274931948
	// y = -0.000000000000000000427519x^5
	// + 0.000000000000021949593800x^4
	// - 0.000000000448278935315231x^3
	// + 0.000004585743992229800000x^2
	// - 0.0278489050237028x
	// + 92.3720978979679
	adcCountscalc = adcCounts;
	calculatedTemperature = (-0.000000000000000000427519 * adcCountscalc * adcCountscalc * adcCountscalc * adcCountscalc * adcCountscalc) +
			(0.000000000000021949593800 * adcCountscalc * adcCountscalc * adcCountscalc * adcCountscalc) -
			(0.000000000448278935315231 * adcCountscalc * adcCountscalc * adcCountscalc) +
			(0.000004585743992229800000 * adcCountscalc * adcCountscalc) -
			(0.0278489050237028 * adcCountscalc) +
			+ 92.3720978979679;
	*Temperature = (int32)calculatedTemperature * 1000; 					// Temperature is now the thermistor temp in Celsius x 1000
*/
	...
	if (tmIndex == 81)
	{
		*Temperature = -25000;						// Cap at -25degC
	}
	else if (tmIndex == 0)
	{
	...
	tcIndex = tens + 8;
	countLower = TCLookupTable[tcIndex];
	countUpper = TCLookupTable[tcIndex + 1];
...
void LoadBigDisplayBuffer(uint8 *buffer, char message[14])
{
	...
	pixelPosition = (127 - totalSpace) / 2;
...
void LoadMacDisplayBuffer(uint8 *buffer, char message[13])
{
	uint8 pixelPosition = 0;						// Used to compress unused space in two-byte characters
	uint8 totalSpace = 0;							// Used to count total space used by message so we can center it
	uint8 byteToCopy1;
	...
	pixelPosition = (128 - totalSpace) / 2;			// Left margin
